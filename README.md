...
| Restriction (ограничение) | SQLite на устройствах | 0 — не хранится (только ретрансляция) |
...
* Фотоотчёты и ограничения не возвращаются typed‑списками в `/sync/batch` — они доставляются только через DeliveryQueue пакеты; после ACK сервер очищает временные файлы.
...
### Фотоотчёт как атомарный набор (3 фото)
* Клиент (Patient) формирует `PhotoReportSetDto` из трёх снимков и отправляет единым пакетом (`/sync/batch` → DeliveryQueue).
* Сервер не сохраняет отчёты в БД, только ретранслирует и шлёт событие `PhotoReportSetAdded` в `patient-{id}`.
* Клиент (Clinic) получает пакет, кэширует фото локально и отправляет ACK по идентификатору пакета, после чего сервер удаляет временные файлы.

### Ограничения (Restrictions)
* Истина живёт на клиентах; сервер лишь ретранслирует пакеты типа `Restriction` и не хранит их долговременно, не возвращает typed‑делту.
